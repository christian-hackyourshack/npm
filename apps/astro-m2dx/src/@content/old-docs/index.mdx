...
- container - `:::class{.some-class} ... :::` around a list of elements
- leaf - `::class{.some-class}` inside container elements
- text - `:class{.some-class}` inside paragraphs or spans
...
Because lists are not present in Markdown as such (only the list items), style could not be applied to the list as a whole. Therefore, there is a special directive `::list-class` that applies the classes from the directive to the succeeding list, if there is one, i.e.
